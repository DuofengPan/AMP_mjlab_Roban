...
from rsl_rl.training_guard import summarize_returns, total_mini_batches
    ...
    def update(self):  # noqa: C901
        ...
        for sample, sample_amp_policy, sample_amp_expert in zip(generator, amp_policy_generator, amp_expert_generator):
            ...
            if not torch.isfinite(loss):
                skipped_non_finite_batches += 1
                training_abort = True
                training_abort_reason = "non-finite total loss before AMP"
                continue
            ...
            if not torch.isfinite(loss):
                skipped_non_finite_batches += 1
                training_abort = True
                training_abort_reason = "non-finite total loss after AMP"
                continue
